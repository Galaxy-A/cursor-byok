...
#[tokio::test]
async fn assistant_terminated_history_preserves_continuations_across_resumes() {
    let (_directory, store) = fixtures::temp_store().await;
    let model = store
    ...
    provider.push(text_response("resumed again", 500, 12));
    provider.push(text_response("next answer", 550, 12));
    let assets = PromptAssets::load(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("prompt/cursor")
            .as_path(),
    ...
    assert!(tail.message_id.starts_with("runtime:continue:"));
    assert_eq!(
        ...
        "committed history plus the first answer, then the durable continuation"
    );
    let stored = store
    ...
    assert_eq!(
        stored
            .iter()
            .filter(|message| message.message_id == tail.message_id)
            .count(),
        1,
        "the provider-visible continuation must be committed exactly once"
    );

    // Recreate the runtime and resume from the delivered checkpoint.
    drop(registry);
    let assets = PromptAssets::load(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("prompt/cursor")
    ...
    let resumed_again = run(
        ...
        request(
            "tail-conversation",
            &model.model_hash,
            resumed.checkpoints.last().cloned(),
            pb::conversation_action::Action::ResumeAction(pb::ResumeAction::default()),
        ),
    )
    .await;
    assert_eq!(resumed_again.turn_ended, 1);
    let next = run(
        &registry,
        "tail-next",
        user_request(
            ...
            "next question",
            &model.model_hash,
            resumed_again.checkpoints.last().cloned(),
        ),
    )
    .await;
    assert_eq!(next.turn_ended, 1);
    let requests = provider.requests();
    ...
    for pair in requests.windows(2) {
        assert_eq!(
            ...
            "resuming must preserve the complete provider-visible prefix"
        );
    }
    let stored = store
        .load_current_messages(&ConversationId::new("tail-conversation"))
        .await
        .unwrap();
    let continuations = stored
        .iter()
        .filter(|message| message.message_id.starts_with("runtime:continue:"))
        .collect::<Vec<_>>();
    assert_eq!(continuations.len(), 2);
    assert_ne!(
        continuations[0].runtime_event_id,
        continuations[1].runtime_event_id
    );
}
