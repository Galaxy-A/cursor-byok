...
impl Store {
    ...
    pub async fn set_pricing_settings(
        ...
        for (name, price) in [
            ...
            ("output_per_million", settings.output_per_million),
            ("cache_read_per_million", settings.cache_read_per_million),
            ("cache_write_per_million", settings.cache_write_per_million),
        ] {
...
mod tests {
    ...
    async fn token_pricing_settings_persists_and_reads_back() {
        ...
        assert_eq!(reopened.pricing_settings().await.unwrap(), custom);
    }

    #[tokio::test]
    async fn invalid_token_prices_do_not_replace_saved_settings() {
        let directory = tempfile::tempdir().unwrap();
        let url = format!("sqlite://{}", directory.path().join("test.db").display());
        let store = Store::connect(&url).await.unwrap();
        let saved = store
            .set_pricing_settings(TokenPricingSettings::default())
            .await
            .unwrap();
        ...
        for invalid in [-1.0, f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            for field in 0..4 {
                ...
                match field {
                    0 => settings.input_per_million = invalid,
                    1 => settings.output_per_million = invalid,
                    2 => settings.cache_read_per_million = invalid,
                    _ => settings.cache_write_per_million = invalid,
                }
                assert!(matches!(
                    store.set_pricing_settings(settings).await,
                    Err(crate::Error::Config(_))
                ));
                assert_eq!(store.pricing_settings().await.unwrap(), saved);
            }
        }

        let free = TokenPricingSettings {
            input_per_million: 0.0,
            output_per_million: 0.0,
            cache_read_per_million: 0.0,
            cache_write_per_million: 0.0,
        };
        assert_eq!(store.set_pricing_settings(free).await.unwrap(), free);
    }
}
