...
func (s *ProxyService) FetchModelAdapterModels(input ModelAdapterModelsRequest) (ModelAdapterModelsResult, error) {
	...
	for _, endpoint := range buildModelListEndpointCandidates(rule, baseURL) {
		models, err := fetchModelListEndpoint(ctx, rule, endpoint, apiKey, input, s.publicClient)
		if err == nil {
...
func fetchModelListEndpoint(ctx context.Context, rule modelListProviderRule, endpoint string, apiKey string, input ModelAdapterModelsRequest, client *http.Client) ([]string, error) {
	collected := []string{}
	cursor := ""
	for page := 0; page < modelAdapterListMaxPages; page++ {
		...
		payload, err := requestModelListPayload(ctx, rule, requestURL, apiKey, input, client)
		if err != nil {
			return nil, err
		}
...
func requestModelListPayload(
	...
	if client == nil {
		client = &http.Client{Timeout: modelAdapterListTimeout}
	}
