...
export function useConfigTransfer({ showSuccess, showActionError }) {
  ...
  async function exportConfig() {
    ...
    try {
      ...
      showSuccess(`配置已导出到 ${exportedPath}`);
    } catch (error) {
      await showActionError("导出失败", toUserError(error));
    } finally {
      busy.value = false;
    }
  }
  ...
  async function importConfig() {
    if (appState.serviceRunning || appState.backendRunning || appState.proxyRunning) {
      await showActionError("导入失败", "服务运行中不能导入完整配置，请先停止服务");
      return;
    }
    try {
      ...
      showSuccess(`配置导入成功，共导入 ${imported.modelAdapters.length} 个模型`);
    } catch (error) {
      await showActionError("导入失败", toUserError(error));
    } finally {
      busy.value = false;
    }
  }
